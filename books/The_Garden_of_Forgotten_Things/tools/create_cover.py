#!/usr/bin/env python3
"""Cover: The Garden of Forgotten Things — Walled garden in golden dawn, stone sundial, white vine in bloom, misty coast, gold/warm stone/pale green."""
...
def draw_gradient(draw: ImageDraw, width: int, height: int) -> None:
    """Golden dawn gradient — gold at top, warm stone, pale green at bottom."""
    for y in range(height):
        if y < height * 0.3:
            t = y / (height * 0.3)
            c = lerp_color((200, 160, 100), (180, 140, 90), t)
        elif y < height * 0.6:
            t = (y - height * 0.3) / (height * 0.3)
            c = lerp_color((180, 140, 90), (130, 120, 80), t)
        else:
            t = (y - height * 0.6) / (height * 0.4)
            c = lerp_color((130, 120, 80), (70, 90, 60), t)
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Step 1.5: Misty coastal backdrop behind wall
    mist = Image.new("RGBA", (WIDTH, HEIGHT), (0, 0, 0, 0))
    md = ImageDraw.Draw(mist)
    md.ellipse((200, 400, WIDTH - 200, 900), fill=(200, 210, 220, 20))
    for y in range(400, 900, 30):
        mx = int(WIDTH * 0.1 + math.sin(y * 0.02) * 100)
        md.line((mx, y, mx + 400, y), fill=(200, 215, 230, 20), width=20)
