#!/usr/bin/env python3
"""Cover: The Winter Orchid — Gothic college tower with snow and falling orchid petals."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    for i, wx in enumerate(range(cx - 150, cx + 160, 70)):
        wy = cy - h // 4
        draw.rectangle([wx, wy, wx + 50, wy + 90], fill=(180, 160, 100, 120))
        draw.rectangle([wx, wy, wx + 50, wy + 90], outline=(60, 50, 70), width=1)
        draw.arc([wx, wy - 25, wx + 50, wy + 25], 0, 180, fill=(180, 160, 100, 100), width=2)
    ...
    rng_orchid = random.Random(13)
    orchid_cx, orchid_cy = cx, cy + 80
    draw.line([(orchid_cx, orchid_cy + 80), (orchid_cx, orchid_cy - 60)], fill=(60, 100, 60), width=4)
    for px, py in [(0, -40), (-30, -20), (30, -20), (-20, -50), (20, -50)]:
        draw.ellipse([orchid_cx + px - 25, orchid_cy + py - 15, orchid_cx + px + 25, orchid_cy + py + 15], fill=(240, 235, 230, 220))
    draw.ellipse([orchid_cx - 8, orchid_cy - 68, orchid_cx + 8, orchid_cy - 52], fill=(200, 180, 100))
    for _ in range(30):
        sx = orchid_cx + rng_orchid.randint(-80, 80)
        sy = orchid_cy + 60 + rng_orchid.randint(-20, 40)
        sr = rng_orchid.randint(3, 10)
        draw.ellipse([sx - sr, sy - sr, sx + sr, sy + sr], fill=(220, 220, 230, 100))

    rng_snow = random.Random(29)
    for _ in range(200):
        sx = rng_snow.randint(0, WIDTH)
        sy = rng_snow.randint(0, 1800)
        sr = rng_snow.randint(1, 3)
        sa = rng_snow.randint(80, 200)
