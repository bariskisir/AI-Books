#!/usr/bin/env python3
"""Cover: The Gravity Orchestra — Tug silhouetted against Earth limb, red fragment tracks through black, Helios array, space black/Earth blue/alert red."""
...
def draw_orbit_scene(draw: ImageDraw.ImageDraw) -> None:
    ...
    # Red fragment tracks — debris trails
    for _ in range(25):
        fx = random.randint(100, 1500)
        fy = random.randint(200, 1300)
        for seg in range(6):
            alpha = max(0, 100 - seg * 15)
            draw.line((fx - seg * 20, fy - seg * 8, fx - (seg + 1) * 20, fy - (seg + 1) * 8), fill=(255, 60, 40, alpha), width=3)

    # Helios array — solar panels near the tug
    for sx in [550, 850]:
        for sy in range(760, 860, 20):
            draw.line((sx, sy, sx + 60, sy), fill=(100, 180, 220, 150), width=2)
        draw.line((sx, 760, sx, 860), fill=(60, 100, 140, 150), width=2)
