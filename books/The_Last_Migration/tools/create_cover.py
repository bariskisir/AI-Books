#!/usr/bin/env python3
"""Cover: The Last Migration — Woman and young boy silhouetted against enormous white wall stretching across gray ash wasteland."""
...
def build(metadata_path: Path, output_path: Path) -> None:
    ...
    for dune in range(5):
        ...
        for i in range(21):
            x = int(WIDTH * i / 20)
            wave = 40 * __import__("math").sin(i * 0.5 + dune * 0.7)
            y_var = rng.randint(-30, 30) + dune * 15
            pts.append((x, base_y + int(wave) + y_var))
        pts.append((WIDTH, HEIGHT))
        dark = 60 + dune * 8
        draw.polygon(pts, fill=(dark, dark - 3, dark - 6))

    # Enormous white wall stretching across horizon
    wall_x = int(WIDTH * 0.10)
    wall_w = int(WIDTH * 0.80)
    wall_base = 720
    wall_top = 280
    # Wall body
    draw.rectangle([wall_x, wall_top, wall_x + wall_w, wall_base], fill=(210, 205, 200))
    # Panel seams
    for i in range(1, 9):
        sx = wall_x + int(wall_w * i / 9)
        draw.line([(sx, wall_top + 15), (sx, wall_base)], fill=(185, 180, 175), width=2)
    # Gate in wall
    gate_w = 100
    gate_x = wall_x + (wall_w - gate_w) // 2
    draw.rectangle([gate_x, wall_base - 180, gate_x + gate_w, wall_base], fill=(60, 55, 50))
    # Horizon line
    draw.line([(0, wall_base), (WIDTH, wall_base)], fill=(180, 170, 160), width=3)

    # Woman and young boy silhouettes in foreground
    # Woman
    wx, wy = WIDTH // 2 - 60, 1180
    draw.ellipse([wx - 6, wy - 20, wx + 6, wy], fill=(25, 22, 20))
    draw.ellipse([wx - 5, wy - 30, wx + 5, wy - 18], fill=(25, 22, 20))
    draw.line([(wx, wy - 18), (wx, wy + 25)], fill=(25, 22, 20), width=4)
    draw.line([(wx, wy + 5), (wx - 8, wy + 18)], fill=(25, 22, 20), width=2)
    draw.line([(wx, wy + 5), (wx + 8, wy + 18)], fill=(25, 22, 20), width=2)
    # Skirt/dress
    draw.polygon([(wx - 10, wy), (wx + 10, wy), (wx + 15, wy + 35), (wx - 15, wy + 35)], fill=(25, 22, 20))

    # Young boy
    bx, by = wx + 30, 1200
    draw.ellipse([bx - 4, by - 14, bx + 4, by], fill=(25, 22, 20))
    draw.ellipse([bx - 3, by - 20, bx + 3, by - 12], fill=(25, 22, 20))
    draw.line([(bx, by - 12), (bx, by + 15)], fill=(25, 22, 20), width=3)
    draw.line([(bx, by), (bx - 5, by + 12)], fill=(25, 22, 20), width=2)
    draw.line([(bx, by), (bx + 5, by + 12)], fill=(25, 22, 20), width=2)
