#!/usr/bin/env python3
"""Cover: The House of Many Windows — Dark brutalist tower with staggered window grid glowing like a thousand watching eyes."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Concrete sky gradient: storm gray to near-black
    for y in range(HEIGHT):
        t = y / HEIGHT
        gray = int(170 - 150 * t)
        draw.line([(0, y), (WIDTH, y)], fill=(gray, gray + 2, gray + 4, 255))

    # Brutalist tower: massive concrete monolith
    cx = WIDTH // 2
    tw, th = 200, int(HEIGHT * 0.6)
    ty = int(HEIGHT * 0.15)
    # Main tower body
    draw.rectangle([cx - tw // 2, ty, cx + tw // 2, ty + th], fill=(120, 125, 130))
    # Vertical ribs
    for rx in range(cx - tw // 2 + 12, cx + tw // 2, 25):
        draw.rectangle([rx, ty, rx + 4, ty + th], fill=(105, 110, 115))
    # Horizontal floor lines
    for fy in range(ty + 35, ty + th, 40):
        draw.line([(cx - tw // 2, fy), (cx + tw // 2, fy)], fill=(100, 105, 110), width=2)

    # Staggered window grid: glowing yellow like watching eyes
    for fy in range(ty + 55, ty + th - 30, 40):
        stagger = 0 if (fy // 40) % 2 == 0 else 14
        for fx in range(cx - tw // 2 + 18 + stagger, cx + tw // 2 - 18, 28):
            if rng.random() < 0.75:
                ww = rng.randint(10, 16)
                wh = rng.randint(18, 26)
                wx = fx + rng.randint(-2, 2)
                # Glass
                draw.rectangle([wx, fy, wx + ww, fy + wh], fill=(180, 190, 200))
                # Yellow glow
                if rng.random() < 0.5:
                    draw.rectangle([wx, fy, wx + ww, fy + wh], fill=(255, 230, 100, rng.randint(60, 150)))
                # Red pupil (watching eye effect)
                if rng.random() < 0.15:
                    draw.ellipse([wx + ww // 2 - 2, fy + wh // 2 - 2, wx + ww // 2 + 2, fy + wh // 2 + 2], fill=(255, 50, 50, 180))

    # Penthouse
    ph = 45
    draw.rectangle([cx - tw // 2, ty - 12, cx + tw // 2, ty - 12 + ph], fill=(80, 85, 90))
    for gx in range(cx - tw // 2 + 8, cx + tw // 2 - 8, 28):
        draw.rectangle([gx, ty - 6, gx + 20, ty - 12 + ph - 6], fill=(100, 140, 180, 150))

    # Ground-level entrance
    draw.rectangle([cx - 25, ty + th - 60, cx + 25, ty + th], fill=(40, 45, 50))
