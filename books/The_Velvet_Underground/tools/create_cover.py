#!/usr/bin/env python3
"""Cover: The Velvet Underground — Dark violet gradient, cassette tape + vinyl record, amplifier tube glow, abstract sound waves."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    for r in range(30, radius, 15):
        cv = 20 + ((r // 15) % 3) * 5
        draw.ellipse([cx - r, cy - r, cx + r, cy + r], outline=(cv, cv - 5, cv + 5), width=1)
    draw.ellipse([cx - 50, cy - 50, cx + 50, cy + 50], fill=(30, 25, 40))
    draw.ellipse([cx - 8, cy - 8, cx + 8, cy + 8], fill=(10, 5, 15))

    cx2, cy2 = WIDTH // 2, int(HEIGHT * 0.40)
    w, h = 360, 240
    draw.rectangle([cx2 - w // 2, cy2 - h // 2, cx2 + w // 2, cy2 + h // 2], fill=(20, 15, 25))
    for i in range(3):
        draw.rectangle([cx2 - w // 2 - i, cy2 - h // 2 - i, cx2 + w // 2 + i, cy2 + h // 2 + i], outline=(60, 50, 70), width=1)
    draw.rectangle([cx2 - 130, cy2 - 80, cx2 + 130, cy2 + 30], fill=(25, 20, 35))
    hub_y = cy2 - 50
    for hub_x in (cx2 - 80, cx2 + 80):
        draw.ellipse([hub_x - 30, hub_y - 30, hub_x + 30, hub_y + 30], fill=(15, 10, 20))
        draw.ellipse([hub_x - 15, hub_y - 15, hub_x + 15, hub_y + 15], fill=(40, 35, 50))
        draw.ellipse([hub_x - 5, hub_y - 5, hub_x + 5, hub_y + 5], fill=(10, 5, 15))
    draw.rectangle([cx2 - 50, hub_y - 2, cx2 + 50, hub_y + 2], fill=(60, 55, 70))
    ...
    for i in range(5):
        ...
        for g in range(4):
            gr = 30 - g * 5
            ga = 40 - g * 8
            gf = [(255, 150, 50, ga), (200, 80, 40, ga), (150, 40, 60, ga), (100, 20, 80, ga)][g]
            draw.ellipse([tx - gr, cy + 260 - gr, tx + gr, cy + 260 + gr], fill=gf)
        draw.rectangle([tx - 12, cy + 220, tx + 12, cy + 290], fill=(40, 35, 45))

    rng = random.Random(17)
    for _ in range(20):
        sy = rng.randint(int(HEIGHT * 0.15), int(HEIGHT * 0.68))
        sx = rng.randint(100, 400)
        ex = rng.randint(WIDTH - 400, WIDTH - 100)
        amp = rng.randint(5, 20)
        freq = 0.02 + rng.random() * 0.03
        pts = []
        for x in range(sx, ex, 4):
            yo = math.sin(x * freq) * amp
            pts.append((x, sy + yo))
        draw.line(pts, fill=(150, 100, 200, rng.randint(15, 35)), width=1)
