#!/usr/bin/env python3
"""Cover: The Garden of Unearthly Delights — Dark greenhouse with glowing bioluminescent garden, giant moth at center, B-movie magenta/teal/violet."""
...
def create_cover(metadata_path, output_path):
    ...
    # Gradient background: dark magenta -> teal -> violet (B-movie horror)
    draw_gradient(
        draw, WIDTH, HEIGHT,
        [(10, 5, 30), (40, 10, 50), (20, 50, 55), (60, 10, 40)]
    )
    ...
    # Giant moth at center — B-movie style
    moth_cx, moth_cy = WIDTH // 2, 500
    # Wings
    for side in [-1, 1]:
        for wing_y in range(0, 5):
            angle = wing_y / 5 * math.pi
            wing_w = int(200 * math.sin(angle))
            wing_h = int(300 * math.sin(angle + 0.3))
            x0 = moth_cx - wing_w * side - 20
            x1 = moth_cx + wing_w * side + 20
            y0 = moth_cy - wing_h + wing_y * 30
            y1 = moth_cy + wing_h + wing_y * 30
            draw.ellipse((
                min(x0, x1), min(y0, y1),
                max(x0, x1), max(y0, y1),
            ), fill=(200, 50, 150, 180 if side == -1 else 150), outline=(100, 200, 200, 150), width=2)
    # Eyes (glowing)
    for ex in [-15, 15]:
        draw.ellipse((moth_cx + ex - 8, moth_cy - 10, moth_cx + ex + 8, moth_cy + 6), fill=(255, 100, 100, 200))
        draw.ellipse((moth_cx + ex - 4, moth_cy - 6, moth_cx + ex + 4, moth_cy + 2), fill=(255, 200, 200, 220))
    # Body
    draw.polygon([
        (moth_cx - 12, moth_cy - 20), (moth_cx + 12, moth_cy - 20),
        (moth_cx + 10, moth_cy + 80), (moth_cx - 10, moth_cy + 80)
    ], fill=(150, 40, 100, 200))
    ...
    # Greenhouse frame — arched glass roof lines
    glass_color = (60, 80, 100, 60)
    draw.line((0, 0, WIDTH // 2, 350), fill=glass_color, width=4)
    draw.line((WIDTH, 0, WIDTH // 2, 350), fill=glass_color, width=4)
