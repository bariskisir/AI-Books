...
def build(metadata_path: Path, cover_path: Path | None, output_path: Path) -> None:
    ...
    book_dir = metadata_path.resolve().parents[1]
    txt_path = book_dir / metadata["paths"]["manuscript"]
...
def main() -> None:
    ...
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    output_path = rel(args.out) if args.out else rel(Path(metadata["paths"]["epub"]))
