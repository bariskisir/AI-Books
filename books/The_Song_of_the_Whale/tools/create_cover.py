#!/usr/bin/env python3
"""Cover: Blue ocean with whale breaching at golden hour, hydrophone cable descending into dark depths, ocean blue/whale gray/sunset gold."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Ocean gradient: golden hour surface to deep blue
    for y in range(HEIGHT):
        if y < 500:
            t = y / 500
            c = tuple(int(a + (b - a) * t) for a, b in zip((180, 100, 40), (100, 130, 160)))
        elif y < 900:
            t = (y - 500) / 400
            c = tuple(int(a + (b - a) * t) for a, b in zip((100, 130, 160), (20, 60, 110)))
        else:
            t = (y - 900) / 1660
            c = tuple(int(a + (b - a) * t) for a, b in zip((20, 60, 110), (5, 20, 50)))
        draw.line([(0, y), (WIDTH, y)], fill=c)

    # Golden sunset reflection on surface
    for x in range(0, WIDTH, 3):
        for dy in range(380, 520, 6):
            wave = math.sin(x * 0.03 + dy * 0.02) * 10
            alpha = int(40 * (math.sin(x * 0.05 + dy * 0.03) * 0.5 + 0.5))
            draw.point((x, dy + int(wave)), fill=(255, 200, 100, alpha))

    # Whale breaching (center, leaping out of water)
    whale_cx, whale_base = WIDTH // 2, 520
    # Body arc
    body = []
    for t in range(0, 200, 3):
        rad = math.radians(t)
        x = whale_cx - 250 * math.cos(rad) * 0.6
        y = whale_base - 180 * math.sin(rad) + 60 * math.cos(rad * 1.3)
        body.append((x, y))
    if len(body) > 2:
        draw.line(body, fill=(160, 180, 200), width=28)
        # highlight
        hl = []
        for t in range(10, 170, 3):
            rad = math.radians(t)
            x = whale_cx - 240 * math.cos(rad) * 0.6
            y = whale_base - 175 * math.sin(rad) + 58 * math.cos(rad * 1.3)
            hl.append((x, y))
        if len(hl) > 2:
            draw.line(hl, fill=(200, 220, 240), width=10)

    # Tail
    tx, ty = whale_cx - 100, whale_base + 50
    draw.polygon([(tx, ty), (tx - 70, ty - 35), (tx - 60, ty + 10), (tx - 70, ty + 40)],
                 fill=(140, 165, 190))

    # Blowhole spray
    for _ in range(25):
        sx = whale_cx + 40 + random.randint(-25, 25)
        sy = whale_base - 140 - random.randint(10, 70)
        sz = random.randint(3, 8)
        draw.ellipse([sx - sz, sy - sz, sx + sz, sy + sz], fill=(200, 230, 255, random.randint(60, 180)))

    # Splash at waterline
    for _ in range(40):
        sx = whale_cx + random.randint(-200, 200)
        sy = whale_base + random.randint(20, 80)
        sz = random.randint(3, 6)
        draw.ellipse([sx - sz, sy - sz, sx + sz, sy + sz], fill=(220, 240, 255, random.randint(40, 120)))

    # Hydrophone cable descending from left
    cable_start = (150, 450)
    for y in range(cable_start[1], 1800, 5):
        offset = math.sin(y * 0.015) * 18 + math.sin(y * 0.03) * 6
        draw.line([(cable_start[0] + offset, y), (cable_start[0] + offset + math.sin(y * 0.015) * 2, y + 5)],
                  fill=(100, 130, 150), width=3)

    # Hydrophone device
    hp_y = 1600
    hp_x = cable_start[0]
    draw.rectangle([hp_x - 12, hp_y, hp_x + 12, hp_y + 50], fill=(120, 150, 170))
    draw.ellipse([hp_x - 18, hp_y + 50, hp_x + 18, hp_y + 70], fill=(160, 185, 200))
    draw.ellipse([hp_x - 10, hp_y + 54, hp_x + 10, hp_y + 66], fill=(190, 210, 225))

    # Sound wave rings from hydrophone
    for i in range(4):
        r = 40 + i * 35
        draw.ellipse([hp_x - r, hp_y + 60 - r, hp_x + r, hp_y + 60 + r],
                     outline=(80, 160, 220, 80 - i * 15), width=2)
