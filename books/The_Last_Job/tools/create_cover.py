#!/usr/bin/env python3
"""Cover: The Last Job — Old safecracker at night looking at glass skyscraper, leather tool bag, rain-glazed streets."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Midnight blue night sky gradient
    for y in range(HEIGHT):
        t = y / HEIGHT
        r = int(5 + 15 * t)
        g = int(5 + 10 * t)
    ...
    # Glass skyscraper (tall, lit windows)
    sx, sy = WIDTH // 2 + 150, int(HEIGHT * 0.10)
    sw, sh = 200, int(HEIGHT * 0.55)
    # Building
    draw.rectangle([sx, sy, sx + sw, sy + sh], fill=(15, 18, 25))
    # Window grid
    for row in range(20):
        for col in range(6):
            wx = sx + 12 + col * 30
            wy = sy + 15 + row * 28
            if rng.random() > 0.3:
                draw.rectangle([wx, wy, wx + 18, wy + 18], fill=(200, 210, 220, rng.randint(60, 180)))
    # Glass reflections
    for i in range(3):
        rx = sx + 20 + i * 60
        draw.polygon([(rx, sy + 20), (rx + 30, sy + 20), (rx + 10, sy + 200), (rx - 20, sy + 200)], fill=(200, 220, 255, 15))

    # City skyline silhouettes (smaller buildings)
    for bx, bw, bh in [(50, 80, 300), (130, 60, 250), (190, 90, 350), (280, 70, 200),
                        (450, 100, 400), (550, 80, 280), (900, 90, 350), (1000, 70, 250),
                        (1100, 100, 380), (1300, 80, 300), (1400, 70, 250), (1500, 80, 280)]:
        by = int(HEIGHT * 0.65) - bh
        draw.rectangle([bx, by, bx + bw, int(HEIGHT * 0.65)], fill=(10, 8, 15))
        # Occasional lit window
        if rng.random() > 0.6:
            for _ in range(rng.randint(2, 5)):
                wx = bx + rng.randint(5, bw - 10)
                wy = by + rng.randint(5, bh - 15)
                draw.rectangle([wx, wy, wx + 4, wy + 6], fill=(60, 55, 40, 150))

    # Old safecracker silhouette (foreground, looking up)
    fx, fy = WIDTH // 2 - 80, int(HEIGHT * 0.58)
    # Body (trench coat)
    draw.polygon([(fx - 25, fy + 60), (fx - 20, fy), (fx + 20, fy), (fx + 25, fy + 60)], fill=(5, 5, 8))
    draw.polygon([(fx - 30, fy + 10), (fx - 20, fy - 10), (fx + 20, fy - 10), (fx + 30, fy + 10)], fill=(5, 5, 8))
    # Head (looking up at skyscraper)
    draw.ellipse([fx - 10, fy - 30, fx + 10, fy - 8], fill=(5, 5, 8))
    # Fedora
    draw.ellipse([fx - 18, fy - 34, fx + 18, fy - 26], fill=(5, 5, 8))
    draw.rectangle([fx - 12, fy - 44, fx + 12, fy - 32], fill=(5, 5, 8))
    # Legs
    draw.rectangle([fx - 16, fy + 60, fx - 6, fy + 95], fill=(5, 5, 8))
    draw.rectangle([fx + 6, fy + 60, fx + 16, fy + 95], fill=(5, 5, 8))

    # Leather tool bag on ground
    bgx, bgy = fx + 40, fy + 70
    draw.rounded_rectangle([bgx, bgy, bgx + 50, bgy + 35], radius=5, fill=(30, 22, 15))
    draw.line([bgx + 5, bgy, bgx + 5, bgy - 8], fill=(40, 32, 25), width=2)
    draw.line([bgx + 45, bgy, bgx + 45, bgy - 8], fill=(40, 32, 25), width=2)
    draw.arc([bgx + 5, bgy - 12, bgx + 45, bgy - 4], 0, 180, fill=(40, 32, 25), width=2)
    # Tools poking out
    for tx in [bgx + 10, bgx + 30]:
        draw.line([tx, bgy, tx, bgy - 15], fill=(100, 95, 85), width=2)

    # Rain-glazed street
    for y in range(int(HEIGHT * 0.65), int(HEIGHT * 0.72), 2):
        shade = 20 + int(8 * __import__("math").sin(y * 0.1))
        draw.line([(0, y), (WIDTH, y)], fill=(shade, shade - 3, shade - 5, 200))
    # Rain streaks
    for _ in range(80):
        rx = int(rng.random() * WIDTH)
        ry = int(rng.random() * HEIGHT)
        rl = int(15 + rng.random() * 30)
        draw.line([(rx, ry), (rx + 3, ry + rl)], fill=(200, 210, 220, rng.randint(10, 30)))

    # Amber streetlight reflections on wet ground
    for _ in range(20):
        lx = int(rng.random() * WIDTH)
        ly = int(HEIGHT * 0.65 + rng.random() * 60)
        ll = int(80 + rng.random() * 120)
        for i in range(5):
            a = max(0, 20 - i * 3)
            draw.ellipse([lx - ll // 2 - i * 5, ly - i * 2, lx + ll // 2 + i * 5, ly + i * 2], fill=(255, 200, 100, a))

    # Amber streetlights
    for lx in [150, WIDTH - 150]:
        ly = int(HEIGHT * 0.35)
        draw.rectangle([lx - 2, ly, lx + 2, ly + 200], fill=(30, 25, 20))
        draw.ellipse([lx - 10, ly - 5, lx + 10, ly + 5], fill=(255, 200, 100, 200))
        for g in range(4):
            gs = 15 + g * 10
            draw.ellipse([lx - gs, ly - gs, lx + gs, ly + gs], fill=(255, 200, 100, 15 - g * 3))
