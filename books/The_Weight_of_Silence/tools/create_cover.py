#!/usr/bin/env python3
"""Cover: The Weight of Silence — Frosted window in empty evaluation room, yellow afternoon light between two empty chairs."""
...
def create_cover(metadata_path: str, out_path: str) -> None:
    ...
    for y in range(ART_HEIGHT):
        t = y / ART_HEIGHT
        if y < 1050:
            r = int(235 + (215 - 235) * t / 0.41)
            g = int(232 + (215 - 232) * t / 0.41)
            b = int(228 + (210 - 228) * t / 0.41)
            draw.line([(0, y), (WIDTH, y)], fill=(r, g, b))
        else:
            s = (y - 1050) / (ART_HEIGHT - 1050)
            r = int(175 + (145 - 175) * s)
            g = int(172 + (142 - 172) * s)
            b = int(168 + (138 - 168) * s)
            draw.line([(0, y), (WIDTH, y)], fill=(r, g, b))
    ...
    for rw in range(16):
        ry = wy + 20 + rw * 36
        if ry < wy + wh:
            draw.line([(wx + 15, ry), (wx + ww - 15, ry)], fill=(255, 252, 248), width=2)
    frame_color = (140, 135, 130)
    draw.rectangle([(wx - 6, wy - 6), (wx + ww + 6, wy + wh + 6)], fill=None, outline=frame_color, width=6)
    draw.line([(wx, wy + wh // 3), (wx + ww, wy + wh // 3)], fill=frame_color, width=4)
    draw.line([(wx, wy + 2 * wh // 3), (wx + ww, wy + 2 * wh // 3)], fill=frame_color, width=4)

    for step in range(35):
        alpha_val = max(0, 30 - step * 0.8)
        ly = 1050 + step * 14
        draw.line([(350, ly), (WIDTH - 50, ly)], fill=(255, 248, 225))

    table_cx, table_ty = WIDTH // 2, 1120
    tw, th = 700, 38
    leg_h = 380
    table_color = (210, 205, 198)
    edge_color = (185, 180, 174)
    for lx in [table_cx - tw // 2 + 30, table_cx + tw // 2 - 30]:
        draw.rectangle([(lx - 7, table_ty + th), (lx + 7, table_ty + th + leg_h)], fill=(145, 140, 135))
    draw.rectangle([(table_cx - tw // 2, table_ty), (table_cx + tw // 2, table_ty + th)], fill=table_color)
    draw.line([(table_cx - tw // 2, table_ty), (table_cx + tw // 2, table_ty)], fill=edge_color, width=3)
    draw.rectangle([(table_cx - tw // 2 + 30, table_ty + 8), (table_cx + tw // 2 - 30, table_ty + 28)], fill=(255, 250, 232))

    for ch_cx, ch_cy in [(470, 1400), (WIDTH // 2, 1100)]:
        ch_w, ch_h = 120, 180
        seat_y = ch_cy - ch_h // 2
        seat_color = (180, 175, 168)
        draw.rectangle([(ch_cx - ch_w // 2, seat_y), (ch_cx + ch_w // 2, seat_y + ch_h)], fill=seat_color)
        draw.rectangle([(ch_cx - ch_w // 2 - 4, seat_y - 4), (ch_cx + ch_w // 2 + 4, seat_y + ch_h + 4)], fill=None, outline=frame_color, width=4)
        draw.rectangle([(ch_cx - ch_w // 2 + 8, seat_y + ch_h), (ch_cx + ch_w // 2 - 8, seat_y + ch_h + ch_h // 3)], fill=seat_color)
        leg_y = seat_y + ch_h + ch_h // 3 + 4
        for lx in [ch_cx - ch_w // 2 + 10, ch_cx + ch_w // 2 - 10]:
            draw.rectangle([(lx - 5, leg_y), (lx + 5, leg_y + 90)], fill=frame_color)
