#!/usr/bin/env python3
"""Cover: Dark Manhattan skyscraper at night, one window-lit floor glowing ominously, elevator panel missing button, night blue/tower glow/button red."""
...
def create_cover(metadata_path, output_path):
    ...
    # --- Elevator panel in foreground ---
    panel_x, panel_y = 100, 1300
    panel_w, panel_h = 120, 300
    draw.rectangle([panel_x, panel_y, panel_x + panel_w, panel_y + panel_h], fill=(40, 38, 35))
    draw.rectangle([panel_x - 3, panel_y - 3, panel_x + panel_w + 3, panel_y + panel_h + 3],
                   outline=(60, 55, 50), width=2)
    # Floor buttons
    for row in range(10):
        for col in range(2):
            bx = panel_x + 15 + col * 50
            by = panel_y + 15 + row * 27
            if row == 8 and col == 0:
                # Missing button — empty hole
                draw.ellipse([bx, by, bx + 28, by + 18], fill=(15, 15, 18))
                draw.ellipse([bx + 2, by + 2, bx + 26, by + 16], fill=(8, 8, 10))
            else:
                draw.ellipse([bx, by, bx + 28, by + 18], fill=(60, 55, 50))
                draw.ellipse([bx + 2, by + 2, bx + 26, by + 16], fill=(50, 48, 45))
                # Button number (tiny)
                num = row * 2 + col + 1
                if num <= 20:
                    draw.text((bx + 8, by + 2), str(num), fill=(200, 200, 200), font=ImageFont.load_default())
    # Red emergency button at bottom
    draw.ellipse([panel_x + 35, panel_y + panel_h - 30, panel_x + 85, panel_y + panel_h - 10], fill=(180, 20, 20))
