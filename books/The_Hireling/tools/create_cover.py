#!/usr/bin/env python3
"""Cover: The Hireling — Sand/shadow gradient, medina silhouette, fedora'd figure with cigarette glow, pale moon."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    for r in range(60, 0, -3):
        draw.ellipse([mx - r, my - r, mx + r, my + r], fill=(220, 210, 190, max(0, 60 - r)))
    draw.ellipse([mx - 40, my - 40, mx + 40, my + 40], fill=(230, 225, 215, 200))

    # Medina skyline silhouette
    rng = __import__("random").Random(17)
    skyline_base = int(HEIGHT * 0.28)
    for x in range(0, WIDTH, 5):
        h = 80 + int(170 * abs(__import__("math").sin(x * 0.012 + 0.5)))
        draw.line([(x, skyline_base), (x, skyline_base - h)], fill=(30, 22, 18))
    # Minaret
    min_x = WIDTH // 3
    draw.rectangle([min_x - 8, skyline_base - 300, min_x + 8, skyline_base], fill=(25, 18, 14))
    draw.polygon([(min_x - 12, skyline_base - 300), (min_x + 12, skyline_base - 300), (min_x, skyline_base - 325)], fill=(25, 18, 14))

    # Back alley perspective
    alley_cx = WIDTH // 2 + 80
    alley_top_y = int(HEIGHT * 0.35)
    alley_bot_y = int(HEIGHT * 0.65)
    for y in range(alley_top_y, alley_bot_y):
        t = (y - alley_top_y) / (alley_bot_y - alley_top_y)
        w = int(60 + 440 * t)
        x1 = alley_cx - w // 2
        x2 = alley_cx + w // 2
        shade = int(15 + 5 * t)
        draw.line([(x1, y), (x2, y)], fill=(shade, shade - 2, shade - 3))

    # Fedora'd figure
    fx, fy = WIDTH // 2 - 60, int(HEIGHT * 0.52)
    # Body
    draw.polygon([(fx - 50, fy + 60), (fx - 30, fy - 60), (fx + 30, fy - 60), (fx + 50, fy + 60)], fill=(10, 8, 6))
    draw.polygon([(fx - 60, fy - 30), (fx - 30, fy - 50), (fx + 30, fy - 50), (fx + 60, fy - 30)], fill=(12, 10, 8))
    # Head
    draw.ellipse([fx - 18, fy - 92, fx + 18, fy - 56], fill=(8, 6, 5))
    # Fedora
    draw.ellipse([fx - 30, fy - 94, fx + 30, fy - 84], fill=(5, 4, 3))
    draw.rectangle([fx - 18, fy - 110, fx + 18, fy - 86], fill=(5, 4, 3))
    # Cigarette glow
    cig_x, cig_y = fx + 25, fy - 55
    draw.rectangle([cig_x, cig_y, cig_x + 12, cig_y + 2], fill=(180, 170, 150))
    draw.ellipse([cig_x, cig_y - 3, cig_x + 6, cig_y + 5], fill=(255, 200, 50, 200))
    for i in range(3):
        draw.ellipse([cig_x + i * 3 - 3, cig_y - 4 - i * 12 - 6, cig_x + i * 3 + 3, cig_y - 4 - i * 12], fill=(200, 190, 170, 30 + i * 10))

    # Ember orange dust motes
    for _ in range(40):
        dx = int(rng.random() * WIDTH)
        dy = int(rng.random() * int(HEIGHT * 0.7))
        ds = rng.randint(1, 3)
        draw.ellipse([dx - ds, dy - ds, dx + ds, dy + ds], fill=(230, 120, 40, rng.randint(20, 60)))
