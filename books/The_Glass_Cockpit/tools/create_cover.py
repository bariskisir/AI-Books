#!/usr/bin/env python3
"""Cover: The Glass Cockpit — Rain-lashed cockpit windscreen at night, instrument panel amber/green glow, dark blue/amber green/storm gray."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Step 4b: Rain on windscreen — streaks and droplets
    rng_ws = random.Random(77)
    for _ in range(40):
        sx = rng_ws.randint(50, WIDTH - 50)
        sy = rng_ws.randint(50, 700)
        angle = rng_ws.uniform(-0.3, 0.3)
        length = rng_ws.randint(30, 100)
        alpha = rng_ws.randint(30, 100)
        draw.line((sx, sy, sx + angle * length, sy + length), fill=(180, 200, 230, alpha), width=2)
    # Windscreen frame
    draw.arc((-100, -50, WIDTH + 100, 800), 0, 180, fill=(15, 18, 25, 200), width=30)
    draw.arc((-80, -40, WIDTH + 80, 780), 0, 180, fill=(25, 30, 40, 150), width=10)
