#!/usr/bin/env python3
"""Cover: The Paleolithic Contract — limestone cave mouth on tropical cliff, half-excavated cranium, headlamp beam, amber/teal tones."""
...
def make_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Amber/teal gradient — limestone cave mouth on tropical cliff
    _draw_gradient(draw, 0, 600, (40, 50, 60), (60, 75, 80))
    _draw_gradient(draw, 600, 1200, (60, 75, 80), (90, 80, 55))
    _draw_gradient(draw, 1200, H, (90, 80, 55), (50, 40, 30))

    # Cave mouth opening — exterior tropical light
    sky_layer, mx, my, mrx, mry = _draw_cave_mouth_opening(draw, rng)
    img = Image.alpha_composite(img, sky_layer)
    draw = ImageDraw.Draw(img, "RGBA")

    # Cave walls framing opening
    ...
    # Half-excavated cranium in sediment — foreground
    cx, cy = W//2, 1400
    draw.ellipse((cx-40,cy-30,cx+40,cy+40), fill=(80,65,50,200))
    draw.ellipse((cx-30,cy-25,cx+30,cy+30), fill=(100,85,65,200))
    # Eye sockets
    draw.ellipse((cx-15,cy-12,cx-5,cy-2), fill=(20,15,10,200))
    draw.ellipse((cx+5,cy-12,cx+15,cy-2), fill=(20,15,10,200))
    # Nasal cavity
    draw.polygon([(cx-4,cy+2),(cx+4,cy+2),(cx,cy+10)], fill=(20,15,10,200))
    # Sediment around cranium
    for _ in range(20):
        sx=cx+rng.randint(-50,50); sy=cy+rng.randint(-35,45)
        draw.ellipse((sx-3,sy-2,sx+3,sy+2), fill=(60,50,35,150))
    # Headlamp beam shining on cranium
    headlamp=Image.new("RGBA",(W,H),(0,0,0,0)); hd=ImageDraw.Draw(headlamp)
    hd.polygon([(800,1200),(cx-30,cy-30),(cx+30,cy-30)], fill=(220,240,255,60))
    headlamp=headlamp.filter(ImageFilter.GaussianBlur(12))
    img=Image.alpha_composite(img,headlamp); draw=ImageDraw.Draw(img,"RGBA")
    # Headlamp source
    draw.ellipse((795,1195,805,1205), fill=(255,255,255,200))
    ...
    # Dust motes
    for _ in range(120):
        x=rng.randint(100,W-100); y=rng.randint(700,1400)
        pr=rng.randint(1,3); pa=rng.randint(30,90)
