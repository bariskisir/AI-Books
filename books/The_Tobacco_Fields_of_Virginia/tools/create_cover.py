#!/usr/bin/env python3
"""Cover: Tobacco field at sunset with lone oak tree, unmarked grave at base, faint ghostly woman in white at field's edge, sunset ochre/tobacco green/ghost white."""
...
def main() -> None:
    ...
    # Unmarked grave at base of oak tree
    grave_x, grave_y = 200, 1390
    draw.ellipse([grave_x - 40, grave_y - 8, grave_x + 40, grave_y + 4], fill=(55, 30, 15))
    draw.rectangle([grave_x - 25, grave_y, grave_x + 25, grave_y + 30], fill=(60, 35, 18))
    draw.rectangle([grave_x - 22, grave_y + 2, grave_x + 22, grave_y + 28], fill=(50, 28, 14))
    # Small rough stone marker
    draw.polygon([(grave_x, grave_y + 30), (grave_x - 8, grave_y + 50), (grave_x + 8, grave_y + 50)],
                 fill=(80, 70, 60))
    # Withered flowers on grave
    for fi in range(3):
        fx = grave_x - 10 + fi * 10
        draw.line([(fx, grave_y), (fx, grave_y - 15)], fill=(100, 60, 40), width=2)
        draw.ellipse([fx - 3, grave_y - 18, fx + 3, grave_y - 12], fill=(120, 50, 40, 120))

    # Faint ghostly woman in white at field's edge
    ghost_x, ghost_y = 1400, 1300
    # Ghostly glow
    for r in range(60, 0, -5):
        a = max(2, 30 - (60 - r) // 3)
        draw.ellipse([ghost_x - r, ghost_y - r - 80, ghost_x + r, ghost_y + r],
                     fill=(220, 230, 240, a))
    # Ghost body (translucent white)
    draw.ellipse([ghost_x - 20, ghost_y - 120, ghost_x + 20, ghost_y - 40], fill=(230, 235, 245, 80))
    draw.line([(ghost_x, ghost_y - 40), (ghost_x, ghost_y)], fill=(230, 235, 245, 60), width=16)
    # Ghost head
    draw.ellipse([ghost_x - 14, ghost_y - 155, ghost_x + 14, ghost_y - 125], fill=(235, 240, 250, 80))
    # Ghost dress trailing
    draw.polygon([(ghost_x - 25, ghost_y), (ghost_x + 25, ghost_y), (ghost_x + 15, ghost_y + 40), (ghost_x - 15, ghost_y + 40)],
                 fill=(230, 235, 245, 50))
    # Faint mist at base
    for _ in range(10):
        mx = ghost_x + random.randint(-30, 30)
        my = ghost_y + random.randint(-10, 30)
        mr = random.randint(8, 18)
        draw.ellipse([mx - mr, my - mr, mx + mr, my + mr], fill=(220, 230, 240, random.randint(15, 35)))
