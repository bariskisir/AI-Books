#!/usr/bin/env python3
"""Cover: The Saffron Kitchen — golden saffron broth steaming in white bowl against dark coastal kitchen window, sea beyond glass."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Dark coastal kitchen interior
    for y in range(HEIGHT):
        t = y / HEIGHT
        if y < HEIGHT * 0.6:
            c = lerp_color((25, 18, 14), (55, 35, 22), y / (HEIGHT * 0.6))
        else:
            c = lerp_color((55, 35, 22), (40, 25, 16), (y - HEIGHT * 0.6) / (HEIGHT * 0.4))
        draw.line([(0, y), (WIDTH, y)], fill=c)

    # Sea through dark kitchen window
    win_y = int(HEIGHT * 0.2)
    draw.rectangle([100, win_y, WIDTH - 100, int(HEIGHT * 0.55)], fill=(50, 80, 100, 120))
    draw.rectangle([100, win_y, WIDTH - 100, int(HEIGHT * 0.55)], outline=(40, 35, 30), width=6)
    # Sea horizon line through window
    sea_gy = int(HEIGHT * 0.35)
    for y in range(win_y + 10, int(HEIGHT * 0.55) - 10):
        t = (y - win_y - 10) / (int(HEIGHT * 0.55) - win_y - 20)
        c = lerp_color((60, 80, 110), (40, 55, 70), t)
        draw.line([(106, y), (WIDTH - 106, y)], fill=c)
    draw.line([(106, sea_gy), (WIDTH - 106, sea_gy)], fill=(120, 160, 180, 80), width=2)
    # Window cross
    draw.line((WIDTH // 2, win_y, WIDTH // 2, int(HEIGHT * 0.55)), fill=(38, 33, 28), width=4)
    draw.line((100, (win_y + int(HEIGHT * 0.55)) // 2, WIDTH - 100, (win_y + int(HEIGHT * 0.55)) // 2), fill=(38, 33, 28), width=4)

    # Counter surface
    cy = int(HEIGHT * 0.58)
    for y in range(cy, int(HEIGHT * 0.70)):
        t = (y - cy) / (int(HEIGHT * 0.70) - cy)
        c = lerp_color((65, 40, 25), (45, 28, 18), t)
        draw.line([(0, y), (WIDTH, y)], fill=c)
    draw.line([(0, cy), (WIDTH, cy)], fill=(140, 100, 55), width=3)

    # White bowl with golden broth
    bx, by = WIDTH // 2, int(HEIGHT * 0.52)
    draw.ellipse([bx - 100, by - 40, bx + 100, by + 50], fill=(220, 215, 205, 230))
    draw.ellipse([bx - 90, by - 32, bx + 90, by + 40], fill=(235, 230, 220, 230))
    draw.ellipse([bx - 75, by - 20, bx + 75, by + 25], fill=(245, 242, 235, 230))
    # Broth inside
    draw.ellipse([bx - 65, by - 10, bx + 65, by + 12], fill=(210, 170, 50, 230))
    draw.ellipse([bx - 50, by - 5, bx + 50, by + 5], fill=(230, 195, 70, 200))

    # Steam rising from broth
    for s in range(3):
        sx = bx - 30 + s * 30
        sy = by - 30
        for i in range(15):
            yy = sy - i * 12
            xx = sx + int(12 * math.sin(i * 0.4 + s))
            alpha = max(0, 70 - i * 5)
            draw.ellipse([xx - 10, yy - 4, xx + 10, yy + 4], fill=(255, 248, 220, alpha))

    # Saffron threads on counter
    rng = random.Random(33)
    for _ in range(40):
        tx = rng.randint(bx - 120, bx + 120)
        ty = rng.randint(cy + 10, int(HEIGHT * 0.68))
        draw.line([(tx, ty), (tx + rng.randint(5, 14), ty + rng.randint(-3, 3))], fill=(200, 40, 18, 200), width=2)
