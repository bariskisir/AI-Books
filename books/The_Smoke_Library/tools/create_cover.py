#!/usr/bin/env python3
"""Cover: Smoke-stained library reading room with green lamp, ruined shelves, yellow police tape billowing in cold wind, smoke brown/lamp green/card yellow."""
...
def make_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Green reading lamp (center)
    lamp_x, lamp_y = W // 2, 600
    # Lamp shade (trapezoid)
    draw.polygon([(lamp_x - 100, lamp_y), (lamp_x + 100, lamp_y),
                  (lamp_x + 60, lamp_y + 80), (lamp_x - 60, lamp_y + 80)],
                 fill=(30, 80, 50, 240), outline=(50, 120, 70, 200))
    # Light cone
    for y in range(lamp_y + 80, 1500, 4):
        t = (y - lamp_y - 80) / 820
        spread = int(60 + t * 300)
        alpha = int(60 * (1 - t * 0.6))
        if alpha < 2:
            break
        draw.line([(lamp_x - spread, y), (lamp_x + spread, y)],
                  fill=(100, 180, 100, alpha))

    # Lamp base
    draw.rectangle([(lamp_x - 20, lamp_y + 80), (lamp_x + 20, lamp_y + 110)], fill=(40, 35, 30))
    draw.ellipse([(lamp_x - 30, lamp_y + 100), (lamp_x + 30, lamp_y + 120)], fill=(50, 45, 40))

    # Reading table
    draw.rectangle([(200, 1200), (1400, 1220)], fill=(80, 60, 40))
    draw.rectangle([(180, 1220), (1420, 1240)], fill=(60, 45, 30))

    # Open book on table
    draw.rectangle([(600, 1100), (1000, 1200)], fill=(160, 140, 100, 200))
    draw.line([(800, 1100), (800, 1200)], fill=(100, 80, 60), width=2)
    # Text lines on book
    for ly in range(1120, 1180, 14):
        draw.line([(620, ly), (780, ly)], fill=(100, 85, 60), width=1)
        draw.line([(820, ly), (980, ly)], fill=(100, 85, 60), width=1)

    # Ruined shelves on left
    for sy in range(500, 1800, 160):
        draw.rectangle([(20, sy), (180, sy + 12)], fill=(120, 80, 50, 200))
        for _ in range(5):
            bx = 30 + random.randint(0, 130)
            bh = random.randint(40, 120)
            draw.rectangle([(bx, sy - bh), (bx + 18, sy)], fill=(90 + random.randint(-20, 30),
                                                                 60 + random.randint(-10, 20),
                                                                 40 + random.randint(-10, 10), 180))

    # Ruined shelves on right
    for sy in range(500, 1800, 160):
        draw.rectangle([(W - 180, sy), (W - 20, sy + 12)], fill=(120, 80, 50, 200))
        for _ in range(5):
            bx = W - 160 + random.randint(0, 120)
            bh = random.randint(40, 120)
            draw.rectangle([(bx, sy - bh), (bx + 18, sy)], fill=(90 + random.randint(-20, 30),
                                                                 60 + random.randint(-10, 20),
                                                                 40 + random.randint(-10, 10), 180))

    # Yellowed card catalog (center-right)
    cat_x, cat_y = 1100, 900
    draw.rectangle([(cat_x, cat_y), (cat_x + 150, cat_y + 300)], fill=(130, 110, 70, 230))
    draw.rectangle([(cat_x + 5, cat_y + 5), (cat_x + 145, cat_y + 295)], fill=(160, 140, 90, 200))
    for row in range(5):
        for col in range(3):
            dx = cat_x + 15 + col * 42
            dy = cat_y + 20 + row * 54
            draw.rectangle([(dx, dy), (dx + 32, dy + 44)], fill=(180, 160, 100), outline=(100, 80, 50), width=1)
            # drawer pull
            draw.ellipse([(dx + 12, dy + 16), (dx + 20, dy + 24)], fill=(60, 50, 40))

    # Yellow police tape billowing in cold wind
    for i in range(6):
        tx = 300 + i * 160
        ty = 1000 + int(40 * math.sin(i * 1.7))
        tape_pts = []
        for t in range(20):
            x = tx + t * 12
            y = ty + int(15 * math.sin(t * 0.8 + i * 2.1))
            tape_pts.append((x, y))
        for j in range(len(tape_pts) - 1):
            draw.line([tape_pts[j], tape_pts[j + 1]], fill=(200, 190, 50, 180), width=6)
        for j in range(len(tape_pts) - 1):
            draw.line([tape_pts[j], tape_pts[j + 1]], fill=(100, 100, 20, 80), width=2)
    # CAUTION DO NOT CROSS text blocks on tape
    for i in range(4):
        tx = 400 + i * 220
        ty = 1020 + int(20 * math.sin(i * 2.3))
        draw.text((tx, ty), "CAUTION", font=font("arialbd.ttf", 18), fill=(180, 170, 40, 160))
    ...
    for _ in range(60):
        ...
        hy = random.randint(200, 1600)
        hr = random.randint(60, 200)
        hd.ellipse([(hx - hr, hy - hr), (hx + hr, hy + hr)],
                    fill=(120, 110, 100, random.randint(8, 25)))
