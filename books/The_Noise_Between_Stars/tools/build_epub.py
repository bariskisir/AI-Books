...
def build(metadata_path: Path, cover_path: Path | None, output_path: Path) -> None:
    ...
    txt_rel = paths.get("txt", metadata.get("TXT_path"))
    txt_path = rel(Path(txt_rel))
    ...
    long_desc = metadata.get("longDescription") or metadata.get("long_description")
    description = long_desc or metadata.get("description") or metadata.get("short_description", "")
...
def main() -> None:
    ...
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    paths = metadata.get("paths", {})
    epub_rel = paths.get("epub", metadata.get("EPUB_path"))
    output_path = rel(args.out) if args.out else rel(Path(epub_rel))
