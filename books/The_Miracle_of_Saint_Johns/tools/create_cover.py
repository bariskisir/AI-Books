#!/usr/bin/env python3
"""Cover: The Miracle of Saint John's — church facade, bell tower, cross, stained glass, warm stone to night blue gradient."""
...
def create_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    # Warm stone gray to night blue gradient
    for y in range(HEIGHT):
        if y < HEIGHT*0.5:
            t=y/(HEIGHT*0.5)
            r=int(140-50*t); g=int(130-60*t); b=int(120-50*t)
        else:
            t=(y-HEIGHT*0.5)/(HEIGHT*0.5)
            r=int(90-60*t); g=int(70-50*t); b=int(70-30*t)
        draw.line([(0,y),(WIDTH,y)], fill=(max(0,r),max(0,g),max(0,b)))

    # Church facade
    cx=WIDTH//2; base_y=int(HEIGHT*0.65)
    body_w, body_h = 380, 300
    draw.rectangle([cx-body_w//2,base_y-body_h,cx+body_w//2,base_y], fill=(100,90,85), outline=(70,60,55), width=2)
    # Bell tower
    tw, th = 90, 220
    tx=cx-body_w//2-tw+20
    draw.rectangle([tx,base_y-body_h-th+50,tx+tw,base_y-body_h+50], fill=(105,95,88), outline=(70,60,55), width=2)
    draw.polygon([(tx-8,base_y-body_h-th+50),(tx+tw//2,base_y-body_h-th+50-70),(tx+tw+8,base_y-body_h-th+50)], fill=(80,70,65))
    # Cross on spire
    cx_t=tx+tw//2; ct=base_y-body_h-th+50-70-25
    draw.line([(cx_t,ct),(cx_t,ct+22)], fill=(200,180,120), width=4)
    draw.line([(cx_t-10,ct+10),(cx_t+10,ct+10)], fill=(200,180,120), width=4)
    # Roof
    draw.polygon([(cx-body_w//2-15,base_y-body_h),(cx,base_y-body_h-70),(cx+body_w//2+15,base_y-body_h)], fill=(80,70,60))
    # Stained glass rose window
    rw_x, rw_y = cx-60, base_y-body_h+40
    draw.ellipse((rw_x,rw_y,rw_x+120,rw_y+120), fill=(100,90,85), outline=(70,60,55), width=2)
    rng=__import__('random').Random(17)
    colors=[(200,50,50,180),(50,80,200,180),(200,180,50,180),(50,180,80,180),(180,100,50,180),(150,50,180,180)]
    for angle in range(0,360,15):
        rad=math.radians(angle)
        x1=cx+8*math.cos(rad); y1=rw_y+60+8*math.sin(rad)
        x2=cx+55*math.cos(rad); y2=rw_y+60+55*math.sin(rad)
        draw.line([(x1,y1),(x2,y2)], fill=rng.choice(colors), width=3)
    # Stained glass glow
    glow=Image.new("RGBA",(WIDTH,HEIGHT),(0,0,0,0)); gd=ImageDraw.Draw(glow)
    gd.ellipse((rw_x-30,rw_y-30,rw_x+150,rw_y+150), fill=(255,200,100,60))
    glow=glow.filter(ImageFilter.GaussianBlur(20))
    img=Image.alpha_composite(img,glow); draw=ImageDraw.Draw(img)
    # Church entrance door
    door_w, door_h = 50, 90
    draw.arc([cx-door_w//2,base_y-door_h,cx+door_w//2,base_y], 0,180, fill=(60,50,45), width=3)
    draw.rectangle([cx-door_w//2,base_y-door_h//2,cx+door_w//2,base_y], fill=(50,40,35))
