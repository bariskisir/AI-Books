...
BOOK_TITLE = "The Echo Thief"
AUTHOR = "Baris Kisir"
OUTPUT = os.path.join(os.path.dirname(__file__), "..", "covers", "The_Echo_Thief.png")
...
def generate_cover():
    ...
    for _ in range(60):
        ...
        color = "hsl(" + f"{hue},{sat}%,{lig}%)"
        shape = rng.choice(["rect","ellipse","triangle"])
        if shape == "rect": draw.rectangle([x,y,x+s,y+s], fill=color)
        elif shape == "ellipse": draw.ellipse([x,y,x+s,y+s], fill=color)
        else: draw.polygon([(x,y),(x+s,y),(x+s//2,y-s)], fill=color)
