#!/usr/bin/env python3
"""Build only The Moonlit Index cover from its generated artwork."""
...
def make_cover(metadata_path: Path, output_path: Path) -> None:
    ...
    for y in range(680):
        draw.line((0, y, W, y), fill=(4, 8, 20, int(160 * (1 - y / 680))))
    for y in range(2060, H):
        draw.line((0, y, W, y), fill=(4, 8, 16, int(25 + 120 * ((y - 2060) / 500))))
    image = Image.alpha_composite(image, shade.filter(ImageFilter.GaussianBlur(2)))
    _draw_standard_cover_title_panel(image, metadata["title"], metadata.get("author", "Barış Kısır"), metadata.get("model", ""))
