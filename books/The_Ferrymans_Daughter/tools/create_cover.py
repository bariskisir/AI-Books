#!/usr/bin/env python3
"""Cover: The Ferryman's Daughter — Dark underworld night, skeletal ferry on Styx, torches on far shore, skull motif, black/sulfur yellow/ember orange."""
...
def draw_star_field(draw: ImageDraw) -> None:
    ...
    for _ in range(120):
        ...
        draw.ellipse([x, y, x + size, y + size], fill=(200, 180, 120, alpha))
...
def draw_moon(draw: ImageDraw) -> None:
    ...
    r = 60
    # Full moon base
    draw.ellipse([cx - r, cy - r, cx + r, cy + r], fill=(200, 180, 120, 60))
    # Crescent by drawing a slightly offset darker circle over it
    draw.ellipse([cx + 15, cy - r - 10, cx + r + 30, cy + r + 10], fill=(15, 10, 5, 255))
    # Glow
    for gr in range(r + 20, r, -2):
        alpha = max(0, int(40 * (1 - (gr - r) / 20)))
        draw.ellipse([cx - gr, cy - gr, cx + gr, cy + gr], fill=(200, 170, 80, alpha))
...
def draw_styx_river(draw: ImageDraw) -> None:
    ...
    # River surface — black with sulfur yellow tinge
...
def draw_ferry(draw: ImageDraw) -> None:
    ...
    # Lantern glow — ember orange
    for gr in range(25, 5, -2):
        alpha = int(40 * (1 - gr / 25))
        draw.ellipse([lantern_x - gr, lantern_y - gr, lantern_x + gr, lantern_y + gr], fill=(220, 120, 20, alpha))
    # Lantern flame
    draw.ellipse([lantern_x - 3, lantern_y - 6, lantern_x + 3, lantern_y + 1], fill=(255, 160, 40))
...
def draw_far_shore(draw: ImageDraw) -> None:
    ...
    # Torches on far shore — ember orange
    ...
    for tx in torch_positions:
        ...
        for fr in range(30, 5, -3):
            ...
            draw.ellipse([tx - fr, 970 - fr // 2, tx + fr, 970 + fr // 2], fill=(220, 100, 20, alpha))
        # Flame core
        draw.ellipse([tx - 5, 968, tx + 5, 980], fill=(255, 140, 30))
        draw.ellipse([tx - 3, 972, tx + 3, 978], fill=(255, 200, 60))


def draw_skull_motif(draw: ImageDraw) -> None:
    """Draw a subtle skull motif in the sky — sulfur yellow."""
    cx, cy = 300, 350
    # Eyes
    draw.ellipse([cx - 20, cy - 12, cx - 5, cy + 3], fill=(80, 60, 20, 180))
    draw.ellipse([cx + 5, cy - 12, cx + 20, cy + 3], fill=(80, 60, 20, 180))
    # Nose cavity
    draw.polygon([(cx - 5, cy + 8), (cx + 5, cy + 8), (cx, cy + 20)], fill=(80, 60, 20, 180))
    # Mouth
    draw.rectangle([cx - 18, cy + 24, cx + 18, cy + 32], fill=(80, 60, 20, 150))
...
def draw_title_panel(draw: ImageDraw) -> None:
    ...
    panel_color_top = (25, 18, 10)
    panel_color_bot = (15, 10, 5)
...
def create_cover() -> Image.Image:
    ...
    # 1. Gradient background: black to sulfur yellow ember
    draw_gradient(draw, [(5, 3, 0), (15, 8, 2), (30, 15, 5), (50, 25, 8)])
