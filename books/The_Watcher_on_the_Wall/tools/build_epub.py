...
def build(metadata_path: Path, cover_path: Path | None, output_path: Path) -> None:
    ...
    txt_rel = Path(metadata.get("paths", metadata).get("manuscript", metadata.get("TXT_path")))
    txt_path = rel(txt_rel)
    ...
    description = metadata.get("description_long") or metadata.get("description_short", "")
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:namespace:container">
  <rootfiles>
...
def main() -> None:
    ...
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    output_path = rel(args.out) if args.out else rel(Path(metadata.get("paths", metadata).get("epub", metadata.get("EPUB_path"))))
