#!/usr/bin/env python3
"""Build EPUB files from TXT manuscripts.

Two modes:
  1. No args — auto-scan books/ and series/ for all metadata JSONs,
     build EPUBs in parallel.
  2. --metadata / --cover / --out — single-book mode (backward compat).
"""
...
def build(metadata_path: Path, cover_path: Path | None, output_path: Path) -> None:
    ...
    if cover_path:
        ...
        if cover_path.exists():
            ...
            cover_item = f'<meta name="cover" content="cover-image"/>\n    <meta property="schema:accessMode">visual</meta>'
...
def build_book(meta_path: Path) -> None:
    try:
        metadata = json.loads(meta_path.read_text(encoding="utf-8-sig"))
        title = metadata.get("title", meta_path.stem)
        epub_path = rel(Path(metadata.get("EPUB_path", "")))
        cover_path = metadata.get("cover_path", "")
        if not epub_path:
            print(f"  SKIP {title} — no EPUB_path in metadata")
            return
        cover = rel(Path(cover_path)) if cover_path else None
        build(meta_path, cover, epub_path)
        print(f"  OK    {title}")
    except Exception as e:
        print(f"  FAIL  {meta_path.name}: {e}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build EPUBs. No args = auto-scan all books in parallel.")
    parser.add_argument("--metadata", type=Path, help="Path to a single metadata JSON")
    parser.add_argument("--cover", type=Path, help="Path to cover image (single-book mode)")
    parser.add_argument("--out", type=Path, help="Output EPUB path (single-book mode)")
    parser.add_argument("-j", "--jobs", type=int, default=0, help="Parallel workers (0 = auto)")
    parser.add_argument("--force", action="store_true", help="Rebuild even if EPUB exists")
    ...
    if args.metadata:
        ...
        if args.cover:
            ...
            build_book(metadata_path)
        return

    all_meta = scan_all()
    print(f"Found {len(all_meta)} book(s) with metadata")
    ...
    if args.force:
        ...
        for mp in all_meta:
            try:
                metadata = json.loads(mp.read_text(encoding="utf-8-sig"))
                epub_path = rel(Path(metadata.get("EPUB_path", "")))
                if not epub_path.exists():
                    to_build.append(mp)
            except Exception:
                to_build.append(mp)
        skipped = len(all_meta) - len(to_build)
        if skipped:
    ...
    workers = args.jobs or min(os.cpu_count() or 4, len(to_build))
    print(f"Building {len(to_build)} EPUB(s) with {workers} worker(s)...")

    with ProcessPoolExecutor(max_workers=workers) as executor:
        fs = {executor.submit(build_book, p): p for p in to_build}
